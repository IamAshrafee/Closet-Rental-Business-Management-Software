...
import { FiUser, FiBox, FiCalendar, FiDollarSign, FiTruck, FiRepeat, FiFileText, FiHash, FiInfo } from 'react-icons/fi';
...
const InfoPair = ({ label, value, className = '' }) => (
    <div className={`flex justify-between items-center py-2 ${className}`}>
        ...
        <p className="font-medium text-gray-800 text-sm text-right">
            {typeof value === 'string' || typeof value === 'number' ? value : React.cloneElement(value)}
        </p>
    </div>
);
                ...
                >
                    ...
                    <div className="flex justify-between items-center p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">Created: {formatDateTime(createdAt)}</p>
                        </div>
                        <button 
                            type="button" 
                            onClick={onClose} 
                    ...
                    <div className="overflow-y-auto flex-grow p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            {customer && (
                                <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <InfoPair label="Address" value={customer.address || 'N/A'} />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <InfoPair label="Rent Period" value={`${formatDate(startDate)} → ${formatDate(endDate)}`} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            {detailedItems.length > 0 && (
                                <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
                                    <SectionHeader icon={<FiBox />} title={`Items (${detailedItems.length})`} />
                                    <div className="space-y-3">
                                        {detailedItems.map((item, index) => (
                                            ...
                                            >
                                                <div>
                                                    <p className="font-medium text-gray-700 text-sm">{item.name || 'Unknown Item'}</p>
                                                    {item.description && (
                                                        <p className="text-xs text-gray-500 mt-1">{item.description}</p>
                                                    )}
                                                </div>
                                                <p className="text-sm font-semibold text-gray-800 whitespace-nowrap">
                                                    {currency.symbol}{parseFloat(item.calculatedPrice || 0).toFixed(2)}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
                                <SectionHeader icon={<FiDollarSign />} title="Payment Summary" />
                                <div className="space-y-2">
                                    <InfoPair label="Subtotal" value={`${currency.symbol}${detailedItems.reduce((sum, i) => sum + parseFloat(i.calculatedPrice || 0), 0).toFixed(2)}`} />
                                    ...
                                    <div className="border-t border-gray-200 my-2 pt-2">
                                        <InfoPair 
                                            label="Total" 
                                            value={`${currency.symbol}${parseFloat(totalAmount || 0).toFixed(2)}`} 
                                    ...
                                    {advances && advances.map((adv, index) => (
                                        <InfoPair 
                                            ...
                                            value={`- ${currency.symbol}${parseFloat(adv.amount || 0).toFixed(2)}`} 
                                    ...
                                    <div className="border-t border-gray-200 my-2 pt-2">
                                        <InfoPair 
                                            label="Balance Due" 
                                            value={`${currency.symbol}${parseFloat(dueAmount || 0).toFixed(2)}`} 
                                            className={`font-bold ${dueAmount > 0 ? 'text-red-600' : 'text-green-600'}`} 
                                        />
                                    </div>
                                </div>
                            </div>
