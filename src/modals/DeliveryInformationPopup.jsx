...
const DeliveryInformationPopup = ({ booking, stockItems, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center backdrop-blur-sm z-50 p-4"
      onClick={onClose}
    >
      <div 
      ...
      >
        ...
        <div className="overflow-y-auto p-6 flex-grow space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 shadow-sm">
            <h3 className="text-lg font-semibold mb-3 text-blue-800 flex items-center"><FiUser className="mr-2" /> Customer Information</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-700 text-sm">
              <p className="flex items-center"><FiInfo className="mr-2 text-blue-500" /> <strong>Name:</strong> {booking.customerName}</p>
              <p className="flex items-center"><FiPhone className="mr-2 text-blue-500" /> <strong>Phone:</strong> {booking.customerPhone}</p>
              <p className="flex items-center sm:col-span-2"><FiMapPin className="mr-2 text-blue-500" /> <strong>Address:</strong> {booking.address}</p>
            </div>
          </div>
        ...
        <div className="flex justify-end space-x-4 p-6 border-t border-gray-200 bg-gray-50">
          <button type="button" onClick={onClose} className="bg-gray-200 text-gray-800 px-6 py-2 rounded-md hover:bg-gray-300 font-semibold transition-colors">Close</button>
        </div>
      </div>
    </div>
  );
